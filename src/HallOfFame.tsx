...
import { getCloudHallProfiles } from "./lib/cloud";
...
export function HallOfFame() {
  ...
  useEffect(() => {
    let active = true;
    Promise.all(accounts.map((account) => loadProgress(account))).then(async (loaded) => {
      const local = { SinbodWayne: loaded[0], KyanWayne: loaded[1] };
      if (!active) return;
      setProfiles(local);
      if (!token) return;
      try {
        const cloud = await getCloudHallProfiles(token);
        if (!active) return;
        setProfiles({
          SinbodWayne: cloud.profiles.SinbodWayne ?? local.SinbodWayne,
          KyanWayne: cloud.profiles.KyanWayne ?? local.KyanWayne,
        });
      } catch {
        // The same-device cache remains useful while offline.
      }
    });
    return () => {
      active = false;
    };
  }, [progress.updatedAt, token]);
  ...
  const copy =
    language === "nl"
      ? {
          ...
          lead: "VOORSPRONG",
          tied: "GELIJK",
          local: token ? "De Hall of Fame vergelijkt de gesynchroniseerde voortgang van beide accounts." : "Offline weergave van de voortgang op dit apparaat.",
        }
      : {
          title: "Hall of Fame",
          ...
          lead: "LEADING",
          tied: "TIED",
          local: token ? "The Hall of Fame compares both accounts' synced progress." : "Offline view of progress saved on this device.",
        };
